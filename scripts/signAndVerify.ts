...
async function main() {
    const  pk = process.env.PRIVATE_KEY_JOSH;
    ...
    if(process.env.REQUEST_MESSAGE) {
        ...
        let derivedAddress = await ethers.utils.verifyMessage(process.env.REQUEST_MESSAGE, sigRequest);
        if(derivedAddress === signer.address) {
            console.log(`signature verified for address ${derivedAddress}`);
        }
    }
}
